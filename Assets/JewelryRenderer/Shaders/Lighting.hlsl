...
// ORIGINAL_END
// CUSTOM_BEGIN
#ifndef JEWELRY_LIGHTING_INCLUDED
#define JEWELRY_LIGHTING_INCLUDED
// CUSTOM_END
...
{
    half NdotL = saturate(dot(normal, lightDir) * .5 + .5);
    return lightColor * NdotL;
}
// CUSTOM_END
    ...
    half clearCoatMask, bool specularHighlightsOff)
{
    ...
    // ORGINAL_END
    // CUSTOM_BEGIN
    // half NdotL = saturate(dot(normalWS, lightDirectionWS) * .5 + .5);
    half NdotL = saturate((dot(normalWS, lightDirectionWS) + 1.) * .5);
    // CUSTOM_END
...
half3 VertexLighting(float3 positionWS, half3 normalWS)
{
    ...
    LIGHT_LOOP_BEGIN(lightsCount)
        ...
        // ORIGINAL_END
        // CUSTOM_BEGIN
        vertexLightColor += LightingHalfLambert(lightColor, light.direction, normalWS);
        // CUSTOM_END
    LIGHT_LOOP_END
#endif
...
half3 CalculateBlinnPhong(Light light, InputData inputData, SurfaceData surfaceData)
{
    ...
    // ORIGINAL_END
    // CUSTOM_BEGIN
    half3 lightColor = LightingHalfLambert(attenuatedLightColor, light.direction, inputData.normalWS);
    // CUSTOM_END
